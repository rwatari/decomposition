...
const render = (image) => {
  ...
  const fragmentShaderSourceNormal = document.getElementById("f-shader-normal").text;
  ...
  const fragmentShaderNormal = createShader(gl, gl.FRAGMENT_SHADER, fragmentShaderSourceNormal);
  ...
  const programNormal = createProgram(gl, vertexShader, fragmentShaderNormal);
  ...
  const resolutionUniformLocationGray = gl.getUniformLocation(programGray, "u_resolution");
  const resolutionUniformLocationNormal = gl.getUniformLocation(programNormal, "u_resolution");
  const resolutionUniformLocationLaplacian = gl.getUniformLocation(programLaplacian, "u_resolution");
  const texSizeUniformLocationLaplacian = gl.getUniformLocation(programLaplacian, "u_textureSize");
  ...
  // load image without flipping upside down
  gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, true);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, image);

  // second texture to render gray scale image
  ...
  const laplacianTexture = createTexture(gl);
  gl.texImage2D(
    gl.TEXTURE_2D, 0, gl.RGBA, image.width, image.height,
    0, gl.RGBA, gl.UNSIGNED_BYTE, null
  );
  ...
  gl.framebufferTexture2D(
    gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, laplacianTexture, 0
  );
  // rendering
  resize(canvas);
  ...
  gl.uniform2f(resolutionUniformLocationGray, image.width, image.height);
  ...
  // render laplacian of image
  gl.useProgram(programLaplacian);
  gl.uniform2f(resolutionUniformLocationLaplacian, image.width, image.height);
  gl.uniform2f(texSizeUniformLocationLaplacian, image.width, image.height);
  // need to rebind texture when swapping framebuffers
  gl.bindTexture(gl.TEXTURE_2D, grayScaleTexture);
  gl.bindFramebuffer(gl.FRAMEBUFFER, FBO2);
  gl.drawArrays(primitiveType, offset, count);

  gl.useProgram(programNormal);
  gl.uniform2f(resolutionUniformLocationNormal, image.width, image.height);
  gl.bindTexture(gl.TEXTURE_2D, laplacianTexture);
